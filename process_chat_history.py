...
def locomo_count_conversations(infile, verbose=False):
    ...
    while not done:
        for section in data:
            section_count += 1
            if verbose:
                print(f'lcc: look for next conversation section {section_count}', file=sys.stderr)
...
def load_locomo(infile, start_time=None, conv_num=None, max_messages=None, verbose=False):
    ...
    while not done:
        for section in data:
            section_count += 1
            if verbose:
                print(f'll: look for next conversation section {section_count}', file=sys.stderr)
            ...
            if 'conversation' in section:
                ...
                for num in range(1, 9999):
                    ...
                    if session_name not in conversation:
                        ...
                        if verbose:
                            print(f'll: finished conversation {conv_count} (1)', file=sys.stderr)
                        break
                    if verbose:
                        print(f'll: loading conversation {conv_count} session {num}', file=sys.stderr)
                    ...
                    try:
                        ...
                        if start_time:
                            if timestamp_compare(start_time, session_time) > 0:
                                if verbose:
                                    print(f'll: skipping old conversation {conv_count} session {num} time={session_time}', file=sys.stderr)
                                break
                    except Exception:
                        if verbose:
                            print(f'll: ERROR: cannot read timestamp of conversation {conv_count} session {num} date={session_date_str}', file=sys.stderr)
                    for message in messages:
                        if 'text' in message:
                            ...
                            if max_messages and msg_count >= max_messages:
                                ...
                                if verbose:
                                    print(f'll: processed max messages={msg_count}', file=sys.stderr)
                                done = True
                                break
                    if done:
                        break
                if verbose:
                    print(f'll: finished conversation {conv_count} sessions={num}', file=sys.stderr)
            if verbose:
